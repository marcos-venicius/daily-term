...
func (e *ResolverError) Error() string {
	if e.Value == nil {
		return "cycleparser.Resolver: nil *Value"
	}
	if e.Kind == Ref && e.Value == e.Value.Value {
		return "cycleparser.Resolver: *Value == (*Value).Value"
	}

	return fmt.Sprintf("cycleparser.Resolver: invalid *Value.Value type: Kind=%q, Type=T%q", e.Kind, e.Type)
}
